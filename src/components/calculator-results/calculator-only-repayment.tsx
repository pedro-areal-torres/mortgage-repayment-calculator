...
export default function CalculatorOnlyRepayment({ calculation, currentTab }: Props) {
    ...
    /* <div className={cn('overflow-hidden rounded-lg bg-white px-4 py-5 shadow-md sm:p-4', currentTab !== 1 && 'hidden')}>
      <dd className="mt-1 text-md font-normal tracking-tight text-gray-900">
        ...
        <div className="flex flex-row gap-1 items-center mt-2">
          ...
          <p className="text-xs leading-6 text-slate-500">{t('Past not included')}</p>
        </div>
        ...
        <div className="text-sm">
          ...
          {formatNumber(calculation.onlyRepayment.totalInterest)}€
        </div>
        <div className="text-sm">
        ...
        <div className="text-sm">
          ...
          {formatNumber(calculation.onlyRepayment.repaymentDetails.repaymentsAmount)}€
        </div>
        <div className="text-sm">
        ...
        <div className="text-sm mt-1">
          ...
          {formatNumber(calculation.onlyRepayment.totalAssets)}€
        </div>
        <div className="text-sm">
          <span className="text-gray-500">{t('Savings')}: </span>
          {formatNumber(calculation.onlyRepayment.totalSaved)}€
        </div>
        <div className="flex flex-row gap-1 items-center mt-2">
          <svg fill="currentColor" color="gray" viewBox="0 0 16 16" height=".8rem" width=".8rem">
            <path d="M8 16A8 8 0 108 0a8 8 0 000 16zm.93-9.412l-1 4.705c-.07.34.029.533.304.533.194 0 .487-.07.686-.246l-.088.416c-.287.346-.92.598-1.465.598-.703 0-1.002-.422-.808-1.319l.738-3.468c.064-.293.006-.399-.287-.47l-.451-.081.082-.381 2.29-.287zM8 5.5a1 1 0 110-2 1 1 0 010 2z" />
                    ...
                    <div className="overflow-hidden shadow ring-1 ring-black ring-opacity-5">
                      <table className="min-w-full divide-y divide-gray-300 text-center">
                        ...
                        <tbody className="divide-y divide-gray-200 bg-white">
                          {calculation.onlyRepayment.paymentDetails.map((detail) => (
                            <tr key={detail.month}>
